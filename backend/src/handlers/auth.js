...
module.exports.postConfirmation = async (event) => {
  ...
  // Best Practice: Fall back to userAttributes.sub to ensure you grab the immutable AWS User UUID
  const userId = userAttributes.sub || event.userName;
  const email = userAttributes.email;
  const name = userAttributes.name || userAttributes.given_name || "New User";

  if (!userId || !email) {
    console.error("Critical failure: Cognito event metadata is missing essential parameters (sub/userName or email).");
    return event;
  }
  ...
  try {
    ...
    // Hardened Choice: We deliberately let this fail so an orphaned user account isn't left out of our database.
    // If you prefer to allow signups anyway, change this to 'return event;' after logging the error.
