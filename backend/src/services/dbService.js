...
/**
 * Removes a specific item from a gift list registry by filtering it out of the array
 * @param {string} listId
 * @param {string} itemId
 */
async function removeItemFromList(listId, itemId) {
  // 1. Fetch current list to identify the target element existence
  const list = await getGiftListById(listId);
  if (!list) throw new Error("List not found");

  const targetItem = list.items.find(i => i.item_id === itemId);
  if (!targetItem) throw new Error("Item not found in list");

  // Safety lock: prevent deletion if someone already purchased it
  if (targetItem.status === "purchased") {
    throw new Error("Cannot delete an item that has already been purchased.");
  }

  // 2. Filter out the targeted item to generate the updated array
  const updatedItems = list.items.filter(i => i.item_id !== itemId);

  const command = new UpdateCommand({
    TableName: TABLE_NAME,
    Key: { list_id: listId },
    UpdateExpression: "SET #items = :updated_items",
    ExpressionAttributeNames: {
      "#items": "items"
    },
    ExpressionAttributeValues: {
  ...
  try {
    ...
    console.error("DynamoDB removeItemFromList error:", error);
